...
struct proc_dir_entry *proc_net_fops_create(struct net *net, const char *name,
                                            umode_t mode, const struct file_operations *fops)
{
    struct proc_dir_entry *entry;

    entry = proc_create(name, mode, net->proc_net, fops);
    if (!entry)
        return NULL;
...
toa_init(void)
{
	...
	if (NULL == ext_stats)
		return 1;
	ptrtoa_stats = proc_net_fops_create(&init_net, "toa_stats", 0, &toa_stats_fops);
